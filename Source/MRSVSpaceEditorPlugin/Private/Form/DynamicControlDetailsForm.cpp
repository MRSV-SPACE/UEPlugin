...
void SDynamicControlDetailsForm::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			...
			[
				SNew(SStaticConfigurationInput)
				.LabelText(FText::FromString("Default Toggled"))
				[
					SNew(SCheckBox)
		...
		[
			...
			[
				...
				[
					SNew(SButton)
					.Text(FText::FromString("Configure Options"))
					.OnClicked_Lambda([this]()
					{
						//Create popup and add handle event
						SControlListFormWidget::ShowAsPopup(MakeShareable(&ControlDetails->Options), FText::FromString("Configure Dropdown Options"), FVector2D(700, 300));
